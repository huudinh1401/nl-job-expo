...
// Tạm ẩn báo cáo quên chấm công khỏi màn duyệt admin, chờ BE cập nhật API mới.
...
const TYPE_OPTIONS = [
    ...
    // Tạm ẩn báo cáo quên chấm công khỏi filter, chờ BE cập nhật API mới.
    ...Object.values(REPORT_TYPES).filter((t) => t.key !== 'attendance').map((t) => ({ key: t.key, label: t.title })),
];
...
const FilterButton = ({ label, onPress }) => (
    <TouchableOpacity onPress={onPress} activeOpacity={0.8} style={{ flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: '#ffffff', borderRadius: 12, paddingHorizontal: 12, height: 42, marginRight: 8, borderWidth: 1, borderColor: '#e2e8f0' }}>
        <Text style={{ flex: 1, color: '#1e293b', fontSize: 12, fontWeight: '600' }} numberOfLines={1}>{label}</Text>
...
const AdminApprovalsScreen = ({ navigation }) => {
    ...
    const ItemView = ({ item }) => {
        ...
        return (
            <View style={{ marginHorizontal: 12, marginVertical: 6, backgroundColor: '#ffffff', borderRadius: 16, padding: 14, borderWidth: 1, borderColor: '#e2e8f0', elevation: 1, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 3 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                    ...
                    <Text style={{ flex: 1, color: '#1e293b', fontSize: 13, fontWeight: '700' }} numberOfLines={1}>{item.username} · {item.part}</Text>
                    <StatusBadge status={item.status} />
                </View>
                <Text style={{ color: '#475569', fontSize: 12, marginLeft: 34 }}>{typeMeta.shortTitle} · {getSummaryLine(item.reportType, item)}</Text>
                {!!item.reason && <Text style={{ color: '#64748b', fontSize: 12, marginTop: 4, marginLeft: 34 }} numberOfLines={2}>{item.reason}</Text>}
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#f8fafc' }}>
            <SafeAreaView style={{ flex: 1, backgroundColor: '#f8fafc' }}>
                ...
                <View style={{ backgroundColor: '#8b5cf6', paddingHorizontal: 15, paddingBottom: 15, borderBottomLeftRadius: 25, borderBottomRightRadius: 25, paddingTop: isAndroid15 ? 35 : 10 }}>
                    ...
                    {loading ? (
                        ...
                        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                            <View style={{ backgroundColor: '#ffffff', padding: 30, borderRadius: 20, alignItems: 'center', borderWidth: 1, borderColor: '#e2e8f0' }}>
                                <Ionicons name="checkmark-done-outline" size={60} color="#cbd5e1" />
                                <Text style={{ color: '#64748b', fontSize: 16, fontWeight: '600', marginTop: 15, textAlign: 'center' }}>Không có đơn nào</Text>
                            </View>
                        </View>
                    )}
                </View>
                ...
                <Modal visible={!!activePicker} transparent animationType="slide" onRequestClose={() => setActivePicker(null)}>
                    <TouchableOpacity style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }} activeOpacity={1} onPress={() => setActivePicker(null)}>
                        <View style={{ backgroundColor: '#ffffff', borderTopLeftRadius: 25, borderTopRightRadius: 25, paddingTop: 20, paddingBottom: 30, maxHeight: '60%' }}>
                            <Text style={{ color: '#1e293b', fontSize: 17, fontWeight: '700', paddingHorizontal: 20, marginBottom: 10 }}>
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) => (
                                    <TouchableOpacity
                                        ...
                                        style={{ paddingVertical: 14, paddingHorizontal: 20, backgroundColor: item.key === pickerValue ? 'rgba(139, 92, 246, 0.12)' : 'transparent' }}
                                    >
                                        <Text style={{ color: item.key === pickerValue ? '#7c3aed' : '#1e293b', fontSize: 15, fontWeight: '600' }}>{item.label}</Text>
                                    </TouchableOpacity>
                                )}
                            />
                        </View>
                ...
                <Modal visible={!!rejectTarget} transparent animationType="fade" onRequestClose={() => setRejectTarget(null)}>
                    <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center' }}>
                        <View style={{ backgroundColor: '#ffffff', margin: 20, borderRadius: 20, padding: 20, width: '90%', borderWidth: 1, borderColor: '#e2e8f0' }}>
                            <Text style={{ color: '#1e293b', fontSize: 17, fontWeight: '700', marginBottom: 12 }}>Lý do từ chối</Text>
                            <TextInput
                                style={{ backgroundColor: '#f8fafc', borderRadius: 12, padding: 14, textAlignVertical: 'top', fontSize: 14, color: '#1e293b', minHeight: 90, borderWidth: 2, borderColor: '#ef4444' }}
                            ...
                            <View style={{ flexDirection: 'row', marginTop: 16 }}>
                                <TouchableOpacity disabled={isMutating} onPress={() => { setRejectTarget(null); setRejectReason(''); }} style={{ flex: 1, backgroundColor: '#f1f5f9', paddingVertical: 12, borderRadius: 12, marginRight: 8, alignItems: 'center' }}>
                                    <Text style={{ color: '#1e293b', fontWeight: '600' }}>Hủy</Text>
                                </TouchableOpacity>
