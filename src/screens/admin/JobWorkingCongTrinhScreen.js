...
import { apiGetJobWorkingCongTrinh } from '../../services/apiService';

const JobWorkingCongTrinhScreen = () => {
    ...
    const getJobWorking = async () => {
        try {
            const data = await apiGetJobWorkingCongTrinh();
            ...
            <View style={{ flex: 1, padding: 4 }}>
                {dataJobWorking.length > 0 ? (
                    ...
                    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                        <Ionicons name="construct-outline" size={60} color="#6b7280" />
                        <Text style={{ color: '#9ca3af', fontSize: 16, fontWeight: '600', marginTop: 15, textAlign: 'center' }}>Không có công việc công trình</Text>
...
export default JobWorkingCongTrinhScreen;
