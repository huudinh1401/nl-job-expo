...
const ModalSelectNV = ({ setVisibleModalSelectNV, users, setIdUserJob, deviceTokenUser, setDeviceTokenUser, handleGiaoJob, setText, text }) => {
    ...
    const handleSelectNV = (id, name, deviceToken) => {
        Alert.alert(
            ...
            [
                ...
                {
                    text: 'OK',
                    onPress: () => { handleGiaoJob(id, deviceToken) },
                },
            ]
        );
        setDeviceTokenUser(deviceToken)
    }

    const handleExitModal = () => {
        Alert.alert(
            'Xác nhận bạn đã giao việc xong',
            `Bấm Ok để đóng!`,
            [
                {
                    text: 'Hủy',
        ...
        return (
            <View style={{ width: (width - 22) / 2, height: 100, margin: 3, backgroundColor: '#2d3748', borderRadius: 16, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3, opacity: statusInfo.disabled ? 0.6 : 1 }}>
                <TouchableOpacity
                    ...
                    onPress={() => {
                        Keyboard.dismiss();
                        handleSelectNV(item.id, item.username, item.device_token)
                    }}
                    activeOpacity={0.7}
                >
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: statusInfo.bgColor, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 }}>
                        <Icon name={statusInfo.icon} type="material" size={16} color={statusInfo.color} style={{ marginRight: 4 }} />
        ...
        <View style={{ width: width - 6, height: height - 100, marginHorizontal: 3, marginVertical: 40, backgroundColor: 'rgba(15, 20, 25, 0.95)', borderRadius: 20, overflow: 'hidden', elevation: 10 }}>
            <View style={{ backgroundColor: '#1a202c', paddingVertical: 16, paddingHorizontal: 20, borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon name="people" type="material" size={24} color="#06b6d4" style={{ marginRight: 8 }} />
            ...
            <View style={{ padding: 16, borderTopWidth: 1, borderTopColor: '#1a202c' }}>
                <TouchableOpacity
                    style={{ width: '100%', justifyContent: 'center', alignItems: 'center', height: 50, backgroundColor: '#f97316', borderRadius: 12, flexDirection: 'row', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 }}
                    onPress={() => handleExitModal()}
                    activeOpacity={0.8}
                >
                    <Icon name="close" type="material" size={20} color="white" style={{ marginRight: 8 }} />
